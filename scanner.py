...
def generate_Dw_for_each_keyword():
    """
    ...
    """
    Dw = {}  # Dw应该是一个dict，方便检索
    distinct_keyword_lst_of_each_doc, distinct_keyword_lst = generate_the_set_of_distinct_keywords_for_docs()
...
def reformat_filename(proj_dir_path=''):
    ...
    for root, dirs, files in os.walk(DIR):
        i = 0
        for file in files:
            os.rename(os.path.join(DIR, file), os.path.join(DIR, str(i) + '.txt'))
            i += 1


if __name__ == '__main__':
    # distinct_keyword_lst_of_each_doc, distinct_keyword_lst = generate_the_set_of_distinct_keywords_for_docs()
    # Dw = generate_Dw_for_each_keyword()
    # print(Dw['thread'])
    # get_s()
    reformat_filename()
